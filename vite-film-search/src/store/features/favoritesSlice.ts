...
const getFavoritesCache = (): MovieInfo[] => {
...
>("favorites/delete", async (movieId, { rejectWithValue, getState }) => {
  try {
    const state = getState();
    const { id } = state.user;
    if (id) await deleteFavoriteFromStore(movieId, id);
    return movieId;
  } catch (error) {
    const firebaseError = error as FirebaseError;
    return rejectWithValue(getFirebaseErrorMessage(firebaseError));
...
const favoritesSlice = createSlice({
  ...
  reducers: {
    removeFavoriteLocal(state, action) {
      const movieId = action.payload as string;
  ...
  extraReducers(builder) {
    ...
    builder.addCase(addFavorite.pending, (state, action) => {
      const movieArg = action.meta.arg as MovieInfo;
    ...
    builder.addCase(addFavorite.rejected, (state, { payload, meta }) => {
      const movieArg = meta.arg as MovieInfo;
    ...
    builder.addCase(deleteFavorite.pending, (state, action) => {
      ...
      if (!state.deletingIds.includes(movieId)) state.deletingIds.push(movieId);
    });
